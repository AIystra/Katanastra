...
public abstract class BloodFluid extends FlowableFluid {
    ...
    public void randomDisplayTick(World world, BlockPos pos, FluidState state, Random random) {
        if (!state.isStill() && !(Boolean)state.get(FALLING)) {
            ...
            world.addParticle(ParticleTypes.EFFECT, (double)pos.getX() + random.nextDouble(),
    ...
    public BlockState toBlockState(FluidState state) {
        return ModBlock.BLOOD_FLUID_BLOCK.getDefaultState().with(FluidBlock.LEVEL, getBlockStateLevel(state));
    }
    ...
    public int getTickRate(WorldView world) {
        return world.getDimension().ultrawarm() ? 20 : 30;
    }
